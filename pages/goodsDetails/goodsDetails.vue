	...
	<view class="fx-gsDetails-container" v-if="Object.keys(productData).length">
		...
		<view class="gs-bottom-wrap">
			<view class="home-tag" @tap="jumpToPage({jumpUrl: '../index/index', switchTab: true})">
				<image src="@/static/images/goodsDetails/tab_home_icon@2x.png"></image>
				<text>首页</text>
			</view>
			<view class="handler-btn">
				...
				<template v-else-if="!loginStatus && queryParams.isShare != 1">
					<view class="buy-share" @tap="jumpToPage({jumpUrl: '../authLogin/authLogin', isLogin: true})">
						<text>分享让好友购买</text>
						<text>赚{{ return_cash }}元</text>
					</view>
				</template>
...
export default {
	...
	methods: {
		...
		jumpOtherApp() {
			...
			if (that.parentPurchaseUrl) {
				purchaseUrl = that.parentPurchaseUrl;
			} else {
				purchaseUrl = that.productShareUrl.purchaseUrl
			}
			if (this.isPg == 1 && this.isCoupon == 1) {
